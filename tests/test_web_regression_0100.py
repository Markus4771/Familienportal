...
def _routes(routes):
    return {(route.path, method) for route in routes for method in (getattr(route, "methods", None) or set())}


def test_core_web_routes_remain_registered():
    routes = _routes(web_router.routes)
    expected = {("/setup", "GET"), ("/setup", "POST"), ("/login", "GET"), ("/login", "POST"), ("/logout", "POST"), ("/dashboard", "GET"), ("/admin", "GET"), ("/admin/users", "POST"), ("/admin/households", "POST")}
    assert expected <= routes


def test_task_web_routes_remain_registered():
    routes = _routes(tasks_router.routes)
    expected = {("/tasks", "GET"), ("/tasks", "POST"), ("/tasks/{task_id}/edit", "GET"), ("/tasks/{task_id}/edit", "POST"), ("/tasks/{task_id}/status", "POST"), ("/tasks/{task_id}/quick-complete", "POST"), ("/tasks/{task_id}/archive", "POST"), ("/tasks/{task_id}/restore", "POST"), ("/tasks/{task_id}/delete", "POST")}
    assert expected <= routes
