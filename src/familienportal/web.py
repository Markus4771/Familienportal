...
@router.post("/setup")
def setup_submit(request: Request, family_name: str = Form(...), family_slug: str = Form(...), household_name: str = Form(...), profile: str = Form("small_family"), admin_name: str = Form(...), admin_email: str = Form(...), admin_password: str = Form(...), db: Session = Depends(get_db)):
    if (db.scalar(select(func.count()).select_from(User)) or 0) > 0:
        return RedirectResponse("/login", status_code=303)
    ...
    db.add_all([family, household]); db.flush()
    ...
    admin.roles.append(roles["Administrator"]); db.add(admin); db.flush()
    session = create_session(db, admin, settings, request.headers.get("user-agent"))
    audit(db, "system.setup.completed", actor=admin, target_type="family", target_id=str(family.id)); db.commit()
    request.session["user_id"] = str(admin.id); request.session["session_id"] = str(session.id)
    return RedirectResponse("/dashboard", status_code=303)


@router.get("/login", response_class=HTMLResponse)
def login_page(request: Request, db: Session = Depends(get_db)):
    if _user_from_session(request, db): return RedirectResponse("/dashboard", status_code=303)
...
def login_submit(request: Request, email: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    mfa = get_mfa_state(db, user.id); requires_mfa = bool(mfa and mfa.enabled) or bool(settings.require_admin_mfa and _is_admin(user))
    if requires_mfa:
        ...
        request.session.clear(); request.session["preauth_user_id"] = str(user.id); return RedirectResponse("/mfa", status_code=303)
    request.session.clear(); session = create_session(db, user, settings, request.headers.get("user-agent")); request.session["user_id"] = str(user.id); request.session["session_id"] = str(session.id)
    user.last_login_at = datetime.now(timezone.utc); audit(db, "auth.login", actor=user, target_type="user", target_id=str(user.id)); db.commit()
    return RedirectResponse("/dashboard", status_code=303)
...
def logout(request: Request, db: Session = Depends(get_db)):
    session_value = request.session.get("session_id")
    if session_value:
        try: item = db.get(LoginSession, UUID(session_value))
        except ValueError: item = None
        if item and item.revoked_at is None: item.revoked_at = datetime.now(timezone.utc); db.commit()
...
def admin_page(request: Request, db: Session = Depends(get_db)):
    user = _user_from_session(request, db)
    if not user: return RedirectResponse("/login", status_code=303)
    if not _is_admin(user): raise HTTPException(status_code=403, detail="Administratorrecht erforderlich")
    users = db.scalars(select(User).where(User.family_id == user.family_id).order_by(User.display_name)).unique().all(); households = db.scalars(select(Household).where(Household.family_id == user.family_id).order_by(Household.name)).all(); roles = db.scalars(select(Role).where(Role.family_id == user.family_id).order_by(Role.name)).all()
...
def admin_add_household(request: Request, name: str = Form(...), description: str = Form(""), db: Session = Depends(get_db)):
    ...
    if not admin or not _is_admin(admin): return RedirectResponse("/login", status_code=303)
    item = Household(family_id=admin.family_id, name=name.strip(), description=description.strip() or None); db.add(item); db.flush(); audit(db, "household.created", actor=admin, target_type="household", target_id=str(item.id)); db.commit(); return RedirectResponse("/admin#households", status_code=303)
...
def admin_add_user(request: Request, display_name: str = Form(...), email: str = Form(...), password: str = Form(...), household_id: str = Form(""), role_name: str = Form("Erwachsene"), db: Session = Depends(get_db)):
    ...
    if db.scalar(select(User).where(User.family_id == admin.family_id, func.lower(User.email) == normalized_email)): return RedirectResponse("/admin?error=Benutzer+existiert+bereits", status_code=303)
    ...
    if role: item.roles.append(role)
    db.add(item); db.flush(); audit(db, "user.created", actor=admin, target_type="user", target_id=str(item.id)); db.commit(); return RedirectResponse("/admin#users", status_code=303)
