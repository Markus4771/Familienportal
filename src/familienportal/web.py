...
def setup_submit(
    ...
    if len(admin_password) < 10:
        return templates.TemplateResponse(
            request=request,
            name="setup.html",
            context=_context(request, error="Das Passwort muss mindestens 10 Zeichen lang sein."),
            status_code=400,
        )
    ...
    db.flush()
    roles = {
        name: Role(family_id=family.id, name=name, permissions=permissions, system_role=True)
...
def login_submit(
    ...
    if not user or user.status != UserStatus.ACTIVE.value or not verify_password(password, user.password_hash):
        return templates.TemplateResponse(
            request=request,
            name="login.html",
            context=_context(request, error="E-Mail-Adresse oder Passwort ist falsch."),
            status_code=401,
        )
    request.session.clear()
...
def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    household_count = (
        db.scalar(select(func.count()).select_from(Household).where(Household.family_id == user.family_id)) or 0
    )
    modules = enabled_modules(db, user.family_id, user)
    connectors = configured_connectors(db, user.family_id)
    settings = family_settings(db, user.family_id)
    return templates.TemplateResponse(
        request=request,
        name="dashboard.html",
        context=_context(
            ...
            navigation_modules=[item for item in modules if item.get("menu")],
        ),
    )
...
def admin_page(request: Request, db: Session = Depends(get_db)):
    ...
    households = db.scalars(
        select(Household).where(Household.family_id == user.family_id).order_by(Household.name)
    ).all()
    roles = db.scalars(select(Role).where(Role.family_id == user.family_id).order_by(Role.name)).all()
    return templates.TemplateResponse(
        request=request,
        name="admin.html",
        context=_context(request, user, users=users, households=households, roles=roles),
    )
...
def admin_add_user(
    ...
    if db.scalar(
        select(User).where(User.family_id == admin.family_id, func.lower(User.email) == normalized_email)
    ):
