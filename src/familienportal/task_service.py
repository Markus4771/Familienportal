...
def archive_task(db,task):
 task.archived_at=datetime.now(timezone.utc);sync_task_event(db,task);db.commit();db.refresh(task);return task
def restore_task(db,task):
 task.archived_at=None;sync_task_event(db,task);db.commit();db.refresh(task);return task
def delete_task(db,task):
 task.status="cancelled";sync_task_event(db,task);db.delete(task);db.commit()
