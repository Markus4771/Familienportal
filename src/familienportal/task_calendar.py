...
def ensure_task_calendar(db: Session, family_id):
    ...
    calendar = Calendar(
        family_id=family_id,
        name=TASK_CALENDAR_NAME,
        slug=TASK_CALENDAR_SLUG,
        kind="tasks",
        description="Fällige Familienaufgaben",
    )
    db.add(calendar)
    db.flush()
    return calendar
...
def task_has_active_calendar_event(task: FamilyTask) -> bool:
    return (
...
def sync_task_event(
    db: Session,
    task: FamilyTask,
    reminder_minutes: int = DEFAULT_TASK_REMINDER_MINUTES,
) -> CalendarEvent | None:
    uid = task_event_uid(task)
    event = db.scalar(
        select(CalendarEvent).where(
            CalendarEvent.family_id == task.family_id,
            CalendarEvent.external_uid == uid,
            CalendarEvent.source == "tasks",
        )
    )

    if not task_has_active_calendar_event(task):
