...
def nextcloud_picker(handle: str, request: Request, path: str = Query(""), db: Session = Depends(get_db)):
    ...
    items = nextcloud_client(state).list_files(path)
    root = nextcloud_client(state)._dav_path("")
    rows = []
    for item in items:
    ...
    return templates.TemplateResponse(request=request, name="genealogy_nextcloud_picker.html", context={"user": user, "handle": handle, "path": path, "parent": parent, "items": rows})
